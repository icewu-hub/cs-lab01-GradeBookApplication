...
namespace GradeBook.UserInterfaces
{
    public static class StartingUserInterface
    {
        ...
        public static void CreateCommand(string command)
        {
            ...
            var name = parts[1];
            var typeofGradebook = parts[2].ToLower();

            BaseGradeBook gradeBook;
            Console.WriteLine(typeofGradebook);
            if (typeofGradebook == "standard")
                gradeBook = new StandardGradeBook(name);
            else if (typeofGradebook == "ranked")
                gradeBook = new RankedGradeBook(name);
            else
                Console.WriteLine($"{typeofGradebook} is not a supported type of gradebook, please try again");return;
